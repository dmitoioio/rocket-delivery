...
> Це проєктна схема. **Реальні міграції написані й перевірені** — [`supabase/migrations/`](../supabase/migrations/), 60 перевірок проти живого Postgres (`npm run db:test`).
> Таблиці живуть в **окремій схемі `delivery`**, не в `public` — [ADR-0011](adr/0011-separate-delivery-schema.md). Звірити назви з CSTL_NEWS не вдалось (доступу немає), а `orders` і `businesses` — найзвичайніші назви, які тільки бувають. Окрема схема робить колізію неможливою, а не малоймовірною.
...
| `waiting_bonus` | `numeric(10,2)` DEFAULT 0 | Надбавка за час висіння в черзі. Протидія черрі-пікінгу (B35) |
...
| `cash_limit` | `numeric(10,2)` | Досяг ліміту → не може брати нові cash-замовлення. **Обовʼязковий** за підрядної моделі (B2, B38) |
| `completed_deliveries` | `int` | Лічильник. Використовується для зниженого ліміту готівки новачкам |
| `payout_details` | `jsonb` | Реквізити виплати (ФОП/картка). Потрібні незалежно від рішення Q11 |
...
Була заготовкою під погодинну оплату. Після рішення від 26.07.2026 (Glovo-подібна модель, чиста відрядна оплата) облік робочого часу не потрібен: ніхто не зобовʼязаний бути онлайн і ніхто не отримує оплату за простій.

`earnings_log.reason = 'hourly'` лишається в переліку як невикористаний — прибирати його з enum дорожче, ніж лишити.
