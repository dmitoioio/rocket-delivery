...
function render() {
  // Перемальовує лише те, що почала людина. Усе інше — таймери,
  // події Realtime, автопілот демо — чекає, поки на екрані є набране.
  //
  // ⚠️ Ознака саме така: «це НЕ дія людини», а не «це перечитування
  // даних». Перша спроба фіксу позначала прапорцем лише перечитування —
  // і не спрацювала: стирало щосекунди, бо перемальовування прилітало
  // ще й з інших джерел. Виміряв, побачив «секунда 1», перевернув умову.
  if (!userAction && hasUnsavedInput()) return;
